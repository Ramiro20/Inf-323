...
public class Cubo {
	...
	private float verticecont[] = new float[] {
			-1, -1, -1, // 0
			1, -1, -1, // 1
			1, 1, -1, // 2
			-1, 1, -1, // 3
	...
	private short indicecont[] = new short[] { 
	...
	private FloatBuffer bufvertcont;
	private ShortBuffer bufindcont;
	...
	public Cubo() {
		...
		ByteBuffer bufB = ByteBuffer.allocateDirect(verticecont.length * 4);
		bufB.order(ByteOrder.nativeOrder()); // Utiliza el orden de byte
												// nativo
		bufvertcont = bufB.asFloatBuffer(); // Convierte de byte a float
		bufvertcont.put(verticecont);
		bufvertcont.rewind(); // puntero al principio del buffer
		
		
		/* Lee los indices */
		bufB = ByteBuffer.allocateDirect(indicecont.length * 2);
		bufB.order(ByteOrder.nativeOrder()); // Utiliza el orden de byte
												// nativo
		bufindcont = bufB.asShortBuffer(); // Convierte de byte a short
		bufindcont.put(indicecont);
		bufindcont.rewind(); // puntero al principio del buffer
	}
	...
	public void dibuja(GL10 gl) {
		...
		/* Se especifica los datos del arreglo de vértices SE UTILIZA EL 3 POR LOS 3 EJES DE COORDENADAS*/
		...
		gl.glVertexPointer(3, GL10.GL_FLOAT, 0, bufvertcont);
		/* Se especifica los datos del arreglo de colores */
		gl.glColor4f(0, 0, 0, 0);
		/* Se dibuja el cubo */
		gl.glLineWidth(10);
		gl.glDrawElements(GL10.GL_LINES, indicecont.length,GL10.GL_UNSIGNED_SHORT, bufindcont
				);
